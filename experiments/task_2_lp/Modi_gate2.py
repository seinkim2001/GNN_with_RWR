...
for fname in tqdm(files):
    ...
    input_dim = x.size(1)
    hidden_dim = input_dim * 3  # ✅ input_dim에 비례하게 설정

    model = GCNEncoderWithGate(input_dim=input_dim, hidden_dim=hidden_dim).to(device)
...
df.to_csv("./results/link_prediction_results_with_gate_dim3x.csv", index=False)
print("✅ 모든 실험 완료! 결과는 ./results/link_prediction_results_with_gate_dim3x.csv 에 저장됨.")
