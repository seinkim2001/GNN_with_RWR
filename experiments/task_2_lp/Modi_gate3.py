...
class GCNEncoderWithGateAndMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(GCNEncoderWithGateAndMLP, self).__init__()

        self.gate = nn.Sequential(
            nn.Linear(input_dim, input_dim),
            nn.LayerNorm(input_dim),     # 안정화
            nn.Sigmoid()
        )
    ...
    def forward(self, x, edge_index):
        gate_weight = self.gate(x)
        x = x * gate_weight              # ✅ selection gate
        x = self.mlp(x)                  # ✅ MLP 통과
        x = self.conv1(x, edge_index)
        x = F.relu(x)
        x = self.conv2(x, edge_index)
...
# ✅ attribute 벡터 경로 지정
attr_dir = "./attributes/generated_50"
...
for fname in tqdm(files):
    ...
    # ✅ hidden_dim 제한 적용 (최대 512)
    max_hidden_dim = 512
    hidden_dim = min(input_dim * 3, max_hidden_dim)

    model = GCNEncoderWithGateAndMLP(input_dim=input_dim, hidden_dim=hidden_dim).to(device)
...
df.to_csv("./results/link_prediction_results_with_gate_dim3x_MLP.csv", index=False)
print("✅ 모든 실험 완료! 결과는 ./results/link_prediction_results_with_gate_dim3x_MLP.csv 에 저장됨.")
